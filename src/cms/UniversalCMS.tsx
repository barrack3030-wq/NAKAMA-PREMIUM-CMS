...
import { logoutCms } from '../auth/AuthGate';
import {
  LayoutDashboard, Image as ImageIcon, Globe, Sliders, Compass, GitBranch, Layers, Menu as MenuIcon, X,
  Eye, Coffee, Calendar, MessageSquare, FileText, HelpCircle, Briefcase, MapPin, Activity, UserCheck,
  CheckCircle2, LogOut
} from 'lucide-react';
...
export const UniversalCMS: React.FC<UniversalCmsProps> = ({ onSwitchToWebsite, onToggleSplitView, isSplitView = false }) => {
  const { config, schema, hasUnpublishedChanges, publishChanges, collections } = useCms();
    ...
    <div className="min-h-screen bg-neutral-100 flex flex-col font-sans" id="universal-cms-root">
      <header className="bg-white border-b border-neutral-200 px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-30 shadow-xs">
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-1.5 text-neutral-500 hover:text-neutral-900 rounded-lg md:hidden" aria-label="Toggle Navigation">
            {mobileMenuOpen ? <X className="w-5 h-5" /> : <MenuIcon className="w-5 h-5" />}
          </button>
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-amber-700 text-white flex items-center justify-center font-bold text-xs shadow-xs">U</div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-bold text-sm tracking-tight text-neutral-900">UniversalCMS</span>
                <span className="text-[10px] bg-neutral-100 text-neutral-600 px-1.5 py-0.5 rounded font-mono font-medium border border-neutral-200">{config.siteType}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button type="button" onClick={handleQuickPublish} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${hasUnpublishedChanges ? 'bg-amber-700 hover:bg-amber-800 text-white shadow-xs' : 'bg-neutral-100 hover:bg-neutral-200 text-neutral-700'}`} id="btn-top-publish"><UploadCloudIcon />Publish</button>
      ...
      <div className="flex-1 flex overflow-hidden">
        <aside className={`w-64 bg-white border-r border-neutral-200 flex flex-col justify-between transition-all duration-200 z-20 ${mobileMenuOpen ? 'fixed inset-y-0 left-0 top-[57px] shadow-xl' : 'hidden md:flex'}`}>
          <div className="p-4 space-y-6 overflow-y-auto">
            <div>
              <p className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider px-3 mb-2">Core CMS</p>
              <nav className="space-y-1">
                <NavButton active={activeTab === 'dashboard'} onClick={() => { setActiveTab('dashboard'); setMobileMenuOpen(false); }} icon={<LayoutDashboard className="w-4 h-4 text-neutral-500" />}>Dashboard</NavButton>
                <NavButton active={activeTab === 'media'} onClick={() => { setActiveTab('media'); setMobileMenuOpen(false); }} icon={<ImageIcon className="w-4 h-4 text-neutral-500" />}>Media Library</NavButton>
              </nav>
            </div>

            <div>
              <div className="flex items-center justify-between px-3 mb-2">
                <p className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">Collections ({enabledCollections.length})</p>
                ...
                {enabledCollections.map(([key, colDef]) => {
                  ...
                  return (
                    <button key={key} type="button" onClick={() => { setActiveTab(`collection-${key}`); setMobileMenuOpen(false); }} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-medium transition-colors cursor-pointer ${isActive ? 'bg-amber-50 text-amber-900 font-semibold' : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900'}`}>
                      <div className="flex items-center gap-2.5"><span className={isActive ? 'text-amber-700' : 'text-neutral-500'}>{getNavIcon(colDef.icon)}</span><span>{colDef.label}</span></div>
                      <span className="text-[11px] text-neutral-400 bg-neutral-100 px-1.5 py-0.5 rounded font-mono">{itemCount}</span>
                    </button>
                  );
                })}
              </nav>
            ...
            <div>
              <p className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider px-3 mb-2">Website Engine</p>
              <nav className="space-y-1">
                <NavButton active={activeTab === 'seo'} onClick={() => { setActiveTab('seo'); setMobileMenuOpen(false); }} icon={<Globe className="w-4 h-4 text-neutral-500" />}>SEO & Social Cards</NavButton>
                ...
                <NavButton active={activeTab === 'navigation'} onClick={() => { setActiveTab('navigation'); setMobileMenuOpen(false); }} icon={<Compass className="w-4 h-4 text-neutral-500" />}>Navigation</NavButton>
                <NavButton active={activeTab === 'publish'} onClick={() => { setActiveTab('publish'); setMobileMenuOpen(false); }} icon={<GitBranch className="w-4 h-4 text-neutral-500" />}>Publish & GitHub</NavButton>
                <NavButton active={activeTab === 'presets'} onClick={() => { setActiveTab('presets'); setMobileMenuOpen(false); }} icon={<Layers className="w-4 h-4 text-neutral-500" />}>Adaptive Schemas</NavButton>
              </nav>
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-8 max-w-6xl w-full mx-auto">
          ...
          {activeTab.startsWith('collection-') && <CollectionManager collectionKey={activeTab.replace('collection-', '')} />}
        </main>
      </div>

      {publishToast && <div className="fixed bottom-5 right-5 bg-neutral-900 text-white text-xs px-4 py-3 rounded-xl shadow-lg flex items-center gap-2 z-50 animate-bounce"><CheckCircle2 className="w-4 h-4 text-emerald-400" /><span>Local CMS state updated.</span></div>}
    </div>
  );
};
...
const NavButton: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; children: React.ReactNode }> = ({ active, onClick, icon, children }) => (
  <button type="button" onClick={onClick} className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-colors cursor-pointer ${active ? 'bg-amber-50 text-amber-900 font-semibold' : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900'}`}>
    {icon}{children}
  </button>
);
